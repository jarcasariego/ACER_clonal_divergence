...
echo "Grouping ${file}..."
        if wc -l ${file} > 0 ; then
                ...
                -c 7 \
                -o count \
                > ${out}DML_Cc.Dc_${feature}.txt
        fi
done


for file in *DML_Cc.Ch*
do
echo "Grouping ${file}..."
        if wc -l ${file} > 0 ; then
                feature=`echo ${file}|cut -f 4 -d '_' | cut -f 1 -d '.'`
                ...
                -c 7 \
                -o count \
                > ${out}DML_Cc.Ch_${feature}.txt
        fi
done


for file in *DML_Ch.Dh*
do
echo "Grouping ${file}..."
        if wc -l ${file} > 0 ; then
                feature=`echo ${file}|cut -f 4 -d '_' | cut -f 1 -d '.'`
                ...
                -c 7 \
                -o count \
                > ${out}DML_Ch.Dh_${feature}.txt
        fi
done

for file in *DML_Dc.Dh*
do
echo "Grouping ${file}..."
        if wc -l ${file} > 0 ; then
                feature=`echo ${file}|cut -f 4 -d '_' | cut -f 1 -d '.'`
