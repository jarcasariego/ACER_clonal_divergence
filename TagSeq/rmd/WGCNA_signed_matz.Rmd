...
# Keep only depth
moduleTraitCor = cor(MEs, traits$depth, use = "p");
moduleTraitPvalue = corPvalueStudent(moduleTraitCor, nSamples);
moduleTraitStars = cut(moduleTraitPvalue, breaks=c(-Inf, 0.001, 0.01, 0.05, Inf), label=c("***", "**", "*", "")) 

# module-trait correlations
pdf("output/depth_module_heatmap.pdf", height=12, width=4)
textMatrix = moduleTraitStars;
par(mar = c(6, 8.5, 3, 3));
# Display the correlation values within a heatmap plot
labeledHeatmap(Matrix = moduleTraitCor,
...
main = paste("Module-trait relationships"))
dev.off()
print(data.frame(table(moduleColors)))
...
whichModule="darkgreen"
top=10 # number of named top-kME genes to plot
...
ggsave(filename="output/heatmap_module_darkgreen.png", phmap, width = 9, height = 2)
...
ggsave(filename="output/heatmap_module_plum.png", phmap, width = 9, height = 2)
...
ggsave(filename="output/heatmap_module_coral2.png", phmap, width = 9, height = 2)
...
```
```{r GO_MWU for coral2 module}
source("data/GO_MWU/gomwu.functions.R")

wdir <- getwd()
# Run GO_MWU for Biological Process (BP) GO terms
commandArgs <- function(...) c("coral2.csv", "BP")
source("data/GO_MWU/GO_MWU.R")

setwd(wdir)
# Run GO_MWU for Molecular Function GO terms
commandArgs <- function(...) c("coral2.csv", "MF")
source("data/GO_MWU/GO_MWU.R")

setwd(wdir)
# Run GO_MWU for all-colony  Cellular Component (CC) GO terms
commandArgs <- function(...) c("coral2.csv", "CC")
source("data/GO_MWU/GO_MWU.R")

setwd(wdir)
# Get list of all significant GO terms for contrast
coral2.GO <- as.list(
  c(BP = "data/GO_MWU/MWU_BP_coral2.csv",
    MF = "data/GO_MWU/MWU_MF_coral2.csv",
    CC = "data/GO_MWU/MWU_CC_coral2.csv")
  ) %>% map(read.table, header = T) %>%
  bind_rows(.id = "ontology") %>%
  as_tibble() %>%
  filter(p.adj < 0.01) %>%
...
# Get list of individual genes with GO annotations and p-values for contrast
coral2.genepvals <- as.list(
  c(BP = "data/GO_MWU/BP_coral2.csv",
    MF = "data/GO_MWU/MF_coral2.csv",
    CC = "data/GO_MWU/CC_coral2.csv")
  ) %>% map(read_tsv) %>%
  bind_rows(.id = "ontology")
...
coral2.GO <- coral2.GO %>%
  mutate(nsig = map_dbl(name, function(.) {
    filter(coral2.genepvals, name == ., -abs(value) <= log10(0.05)) %>%
    nrow(.)
  }))
...
write_tsv(coral2.GO %>%
  unite("genes", nsig, nseqs, sep = "/") %>%
  arrange(ontology, delta.rank), "output/coral2_module_goout.txt")
```
...
# Run Matz code to create gomwuplot
setwd("data/GO_MWU/")
bp <- gomwuPlot(inFile = "coral2.csv", 
   goDivision = "BP", goAnnotations = "acer_iso2go.tab")
cc <- gomwuPlot(inFile = "coral2.csv", 
   goDivision = "CC", goAnnotations = "acer_iso2go.tab")
mf <- gomwuPlot(inFile = "coral2.csv", 
   goDivision = "MF", goAnnotations = "acer_iso2go.tab")
