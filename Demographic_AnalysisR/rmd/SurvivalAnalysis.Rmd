...
a <-ggsurvplot(km_fit, data = SurvData, legend = "none")
a
...
d <- ggsurvplot(km_genot_fit, data = SurvData, pval = TRUE, legend = "none") # legend is too large with all genotypes
d
...
# compare curves by reef
km_reef_fit2 <- survfit(Surv(time, status) ~ Reef, data=SurvData2)
summary(km_reef_fit2, times = c(1,10,20,30*(1:10)))
ggsurvplot(km_reef_fit2, data = SurvData2, pval = TRUE)

# By depth

km_depth_fit2 <- survfit(Surv(time, status) ~ Depth, data=SurvData2)
summary(km_depth_fit2, times = c(1,10,20,30*(1:10)))
ggsurvplot(km_depth_fit2, data = SurvData2, pval = TRUE, legend = "none")


# Genotype

km_genot_fit2 <- survfit(Surv(time, status) ~ Genot, data=SurvData2)
ggsurvplot(km_genot_fit2, data = SurvData2, pval = TRUE, legend = "none")
