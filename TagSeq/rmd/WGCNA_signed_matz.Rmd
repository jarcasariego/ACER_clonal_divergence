...
```
```{r GO_MWU for violet module}
source("data/GO_MWU/gomwu.functions.R")

wdir <- getwd()
# Run GO_MWU for Biological Process (BP) GO terms
commandArgs <- function(...) c("violet.csv", "BP")
source("data/GO_MWU/GO_MWU.R")

setwd(wdir)
# Run GO_MWU for Molecular Function GO terms
commandArgs <- function(...) c("violet.csv", "MF")
source("data/GO_MWU/GO_MWU.R")

setwd(wdir)
# Run GO_MWU for all-colony  Cellular Component (CC) GO terms
commandArgs <- function(...) c("violet.csv", "CC")
source("data/GO_MWU/GO_MWU.R")

setwd(wdir)
# Get list of all significant GO terms for contrast
violet.GO <- as.list(
  c(BP = "data/GO_MWU/MWU_BP_violet.csv",
    MF = "data/GO_MWU/MWU_MF_violet.csv",
    CC = "data/GO_MWU/MWU_CC_violet.csv")
  ) %>% map(read.table, header = T) %>%
  bind_rows(.id = "ontology") %>%
  as_tibble() %>%
  filter(p.adj < 0.01) %>%
...
# Get list of individual genes with GO annotations and p-values for contrast
violet.genepvals <- as.list(
  c(BP = "data/GO_MWU/BP_violet.csv",
    MF = "data/GO_MWU/MF_violet.csv",
    CC = "data/GO_MWU/CC_violet.csv")
  ) %>% map(read_tsv) %>%
  bind_rows(.id = "ontology")
...
violet.GO <- violet.GO %>%
  mutate(nsig = map_dbl(name, function(.) {
    filter(violet.genepvals, name == ., -abs(value) <= log10(0.05)) %>%
    nrow(.)
  }))
...
write_tsv(violet.GO %>%
  unite("genes", nsig, nseqs, sep = "/") %>%
  arrange(ontology, delta.rank), "output/violet_module_goout.txt")
```
...
# Run Matz code to create gomwuplot
setwd("data/GO_MWU/")
bp <- gomwuPlot(inFile = "violet.csv", 
   goDivision = "BP", goAnnotations = "acer_iso2go.tab")
cc <- gomwuPlot(inFile = "violet.csv", 
   goDivision = "CC", goAnnotations = "acer_iso2go.tab")
mf <- gomwuPlot(inFile = "violet.csv", 
   goDivision = "MF", goAnnotations = "acer_iso2go.tab")
