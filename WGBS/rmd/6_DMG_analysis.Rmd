...
#load("output/meth_dataset.RData")
...
#ggsave(filename = "output/Fig_pcoa_DMR.png",pcoa, width = 109, height = 84.5, units = "mm")
...
DM <- aggregate(per.meth ~ genotype*site*gene, data=sub_meth_table[which(sub_meth_table$genotype != " C1733 "), ], FUN=mean) %>% 
  pivot_wider(names_from = site, values_from = per.meth)%>%
  mutate(log2fold = log2(LP15/LP40))

DM_all <- aggregate(per.meth ~ site*gene, data=sub_meth_table[which(sub_meth_table$genotype != " C1733 "), ], FUN=mean) %>% 
  pivot_wider(names_from = site, values_from = per.meth)%>%
  mutate(log2fold = log2(LP15/LP40))%>%
  mutate(genotype = "all") %>%
  bind_rows(DM) %>% 
...
#ggsave(filename = "output/KOG_heatmap.png", KOGfig, width = 195, height = 84.5, units = "mm")
