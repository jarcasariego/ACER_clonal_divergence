...
```{r}
set.seed(1234)
adonis_site <- adonis2 (perc_meth_by_sample100[, c(4:ncol(perc_meth_by_sample100))] ~ site, strata = perc_meth_by_sample100$genotype, data = perc_meth_by_sample100, permutations = 9999, method = "binomial", na.rm = TRUE)

adonis_siteGenot <- adonis2 (perc_meth_by_sample100[, c(4:ncol(perc_meth_by_sample100))] ~ site*genotype, data = perc_meth_by_sample100, permutations = 9999, method = "manhattan",  na.rm = TRUE)
...
jpeg("output/all.aov_0.05_100_DMR_snp_filt_heatmap.jpg", width = 600, height = 1000)
...
genet_color <- data.frame(sample=colnames(aov_0.05_site_DMR_m))
...
jpeg("output/DMR.aov_0.05_100_bysample_SNP_filt_heatmap.jpg", width = 1000, height = 1000)
...
write.csv(all_DMR_100_1way_aov_modelsumm_wide,"output/aov_site_snp_filt.csv", row.names = FALSE, quote = FALSE)
```
...
#write out data
write.table(DMR_100_sig.bed, "output/aov_0.05_site_snp_filt.bed", sep = "\t",row.names = F, col.names = F, quote = F)
...
write.csv(all_DMR_100_2way_aov_modelsumm_wide,"output/aov2way_all_snp_filt.csv", row.names = FALSE, quote = FALSE)
```
...
#write out data
write.table(DMR_100_sig_site_2way.bed, "output/aov2way_0.05_site_snp_filt.bed", sep = "\t",row.names = F, col.names = F, quote = F)
...
#write out data
write.table(DMR_100_sig_genet_2way.bed, "output/aov2way_0.05_genotype_snp_filt.bed", sep = "\t",row.names = F, col.names = F, quote = F)
...
#write out data
write.table(DMR_100_sig_interaction_2way.bed, "output/aov2way_0.05_site:genet_snp_filt.bed", sep = "\t",row.names = F, col.names = F, quote = F)
```
