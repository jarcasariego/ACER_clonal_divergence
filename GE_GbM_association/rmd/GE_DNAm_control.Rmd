...
saveRDS(m, "data/meth_dataset.RData")

## Create a summary of methylation for each gene ##

m <- readRDS("data/meth_dataset.RData")
mean <- rowMeans(m)
...
cv_D <- sd_D/mean_D
# Coefficient of Variation for all samples
sd <- rowSds(e)
cv <- sd/mean
# Coefficient of Variation among treatments
meanAmongTrt <- cbind(mean_S,mean_D)
...
gene_summary <- cbind(mean,sd,cv,mean_S,mean_D,cv_S,cv_D,cv_mean_AmongTrt,
                      diff_Trt)
...
gene_reduce <- data.frame(label=gene_annot$label,gene_annot[,c(11,2:10)])
...
mean_GbMvsGE_deep <- ggplot(df,aes(x=gene_mean_D,y=mean_D*100)) + 
    ...
    labs(x="Gene Expression (log2-cpm)",y="DNA Methylation (%)") +
    theme_cowplot()
df$density <- get_density(df$gene_mean, df$mean*100, n = 100)
mean_GbMvsGE_all <- ggplot(df,aes(x=gene_mean,y=mean*100)) + 
    geom_point(aes(color = density)) + ylim(0,100) + 
    geom_smooth(method=lm,colour="orange",size=2,na.rm = TRUE) +
    scale_color_viridis() +
    labs(x="Gene Expression (log2-cpm)",y="DNA Methylation (%)") +
...
meanGbMvsGEcv_deep <- ggplot(df,aes(log(x=gene_cv_D+0.01),y=mean_D*100)) + 
    ...
    labs(x=bquote("Gene Expression"~CV[Ind]~"(log)"),y="DNA Methylation (%)") +
    theme_cowplot()
df$density <- get_density(log(df$gene_cv+0.01), df$mean*100, n = 100)
meanGbMvsGEcv_all <- ggplot(df,aes(log(x=gene_cv+0.01),y=mean*100)) + 
    geom_point(aes(color = density)) + ylim(0,100) + scale_color_viridis() +
    geom_smooth(method=lm,colour="orange",size=2) +
    labs(x=bquote("Gene Expression"~CV[Ind]~"(log)"),y="DNA Methylation (%)") +
    theme_cowplot()

all_sample_plot <- plot_grid(mean_GbMvsGE_all, meanGbMvsGEcv_all, ncol=2, labels = c("D", "E"))
...
ggsave(all_sample_plot,filename="output/GbM_GE_correlations_allsamples.png", height = 4, width = 9)
### Statistics ###
...
plot(lm(mat$gene_mean~mat$mean))
# Mean Methylation vs. CV Gene Expression
(cpg_cv <- summary(lm(mat$gene_cv~mat$mean)))
...
```{r DEGs expression change vs DNA meth}
# prepare DMGs to filter DM/GE data
DEG <- read.delim("../TagSeq/output/DEGs_all_genets.tsv", sep = "\t")

#keep only gene_id
colnames(DEG) <- c("iso","annot")

# convert iso to transcripts like meth gene_id
iso2seq <- read.delim("../TagSeq/data/ACER_Transcriptome(Libro_etal_2013)/a.cervicornis_Libro etal 2013_annot_Matz/acer_seq2iso.tab", header = FALSE)
colnames(iso2seq) <- c("gene_id", "iso")

DEGs <- merge(DEG, iso2seq)

#Subset DM/GE dataset
mat_red <- subset(mat,select=c("gene_id","diff_Trt","gene_diff_Trt"))
mat_red[,2] <- mat_red[,2]*100 

DEG_join <- left_join(DEGs,mat_red,by="gene_id")
DEG_join <- DEG_join[, -c(1:2)]
DEG_join <- DEG_join[complete.cases(DEG_join),] #eliminate genes for which no Meth data is available


#Create a diff_cv variable for all contrasts 
DEG_join$expresdiff <- ifelse(DEG_join$gene_diff_Trt < 0,
                                      "down-regulated",
                                      "up-regulated")
p1<-ggplot(DEG_join,
           aes(y=diff_Trt,x=gene_diff_Trt,colour=expresdiff)) + 
  geom_smooth(method=lm,colour="orange",size=2,na.rm = TRUE) +
  geom_point(size=3) + ylim(-2.5,2.5) + xlim(-1.5,2) +  
  theme_cowplot() +
  labs(x=bquote("Gene expression change in DEGs (FoldChange"~Log[2]~")"),y="Methylation change (fraction difference)") +
  theme(plot.title = element_text(hjust=0.5), legend.position = "none")

p1

ggsave(p1,filename="output/DEG_meth_correlation.png", height = 4, width = 6)

# Stats

(deg_meth <- summary(lm(DEG_join$diff_Trt~DEG_join$gene_diff_Trt)))
```
